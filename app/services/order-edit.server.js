...
// the subscription price whenever the batch's combined real price can cover it. Real product
// variants are always kept (so inventory, digital delivery and auto-fulfill keep working);
// Shopify order edits can only discount, never surcharge, so a product is never charged above
// its real price.
...
export async function performOrderEdit({ admin, orderGid, targetLineItems, batch, currency, freeRotation = false, keepTargetProduct = false, skipZeroOut = false }) {
  ...
  // Rotation products are ALWAYS added as their real variant (kept for inventory, digital
  // delivery and auto-fulfill); pricing is matched by discounting down — never by surcharge.
  ...
  if (isSingle) {
    ...
    // ── Multi-product batch: even split across the batch's REAL products ───────
    // The original subscription total (sum of all target line item totals, in presentment cents)
    // is divided as EVENLY as possible across the batch products, but no product is charged above
    // its real price (Shopify can only discount). Any share a cheaper product can't absorb is
    // redistributed to the products that still have headroom (allocateEven), so the order total
    // equals the subscription price whenever the batch's combined real price can cover it. Each
    // product is added at qty 1 as its real variant (kept for inventory / digital delivery /
    // auto-fulfill). All arithmetic is in integer cents of the order's presentment currency.
    ...
    //    free → everything is free; otherwise even split capped at each real price.

    ...
    <s-page heading="Subscription Product Rotation">
      ...
      {!setupDone && (
        <s-section heading="Setup Checklist">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <CheckItem done={checklist.hasGroups} label="Add at least one target subscription product" href="/app/rotation-groups/new" />
            <CheckItem done={checklist.hasItems}  label="Add rotation products to each group"          href="/app/rotation-groups" />
            <CheckItem done                        label="orders/create webhook registered (shopify.app.toml)" />
          </div>
        </s-section>
      )}
      ...
      <s-section heading="Quick Actions">
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          ...
          <s-button href="/app/rotation-groups"     variant="secondary">Manage Groups</s-button>
          <s-button href="/app/rotation-logs"       variant="secondary">View Logs</s-button>
        </div>
      </s-section>
      ...
      <s-section heading="Recent Rotations">
        {recentLogs.length === 0 ? (
          <div style={emptyState}>
            ...
            <div style={{ fontSize: "15px", fontWeight: "600", color: "#303030", marginBottom: "6px" }}>No rotations yet</div>
            <div style={{ fontSize: "13px", color: "#6d7175" }}>Rotations appear here after renewal orders are processed</div>
          </div>
        ) : (
          <>
            ...
            <div style={{ paddingTop: "12px" }}>
              <Link to="/app/rotation-logs" style={{ color: "#2c6ecb", textDecoration: "none", fontSize: "13px" }}>View all logs →</Link>
            </div>
          </>
        )}
      </s-section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          {[
            ...
            <div key={i} style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
              ...
              <div style={{ fontSize: "13px", color: "#303030", lineHeight: "1.5", paddingTop: "2px" }}>{text}</div>
            </div>
          ))}
        </div>
      </s-section>
      ...
      <s-section slot="aside" heading="Connected Store">
        <div style={{ background: "#f6f6f7", borderRadius: "6px", padding: "10px 12px", fontFamily: "monospace", fontSize: "12px", color: "#303030", wordBreak: "break-all" }}>
          {shop}
        </div>
      </s-section>
...
function StatCard({ value, label, icon, href }) {
  const card = (
    <div style={statCard}>
      ...
      <div style={{ fontSize: "32px", fontWeight: "700", color: "#303030", lineHeight: "1", marginBottom: "6px" }}>{value}</div>
      <div style={{ fontSize: "12px", color: "#6d7175", fontWeight: "500", textTransform: "uppercase", letterSpacing: "0.4px" }}>{label}</div>
      {href && <div style={{ marginTop: "10px", fontSize: "12px", color: "#2c6ecb" }}>View →</div>}
    </div>
  );
...
function CheckItem({ done, label, href }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 16px", borderRadius: "8px", background: done ? "#f1fbf6" : "#fafafa", border: `1px solid ${done ? "#c3e6cc" : "#e1e3e5"}` }}>
      <span style={{ fontSize: "16px", flexShrink: 0 }}>{done ? "✅" : "⬜"}</span>
      {href && !done
        ? <Link to={href} style={{ fontSize: "13px", color: "#2c6ecb", textDecoration: "none" }}>{label}</Link>
        : <span style={{ fontSize: "13px", color: done ? "#008060" : "#6d7175" }}>{label}</span>
      }
    </div>
  );
}
...
function StatusBadge({ status }) {
  const map = { SUCCESS: { bg: "#e3f5e9", color: "#008060" }, FAILED: { bg: "#fce8e8", color: "#d82c0d" }, SKIPPED: { bg: "#fff5e5", color: "#b98900" } };
...
const stepDot = { minWidth: "22px", height: "22px", borderRadius: "50%", background: "#303030", color: "#fff", fontSize: "11px", fontWeight: "600", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 };
